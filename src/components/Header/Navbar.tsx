...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const checkMediaPhone = () => {
      setIsMediaPhone(window.innerWidth <= 1280); // Adjust the breakpoint as per your requirement
    };
  ...
  return (
    <Fragment>
      {isMediaPhone && ( // Conditionally render the hamburger menu on media phones
        <div
          className="fixed top-4 right-4 z-[99] cursor-pointer"
          onClick={handleMenuToggle}
        >
          {/* Hamburger Icon */}
          <div className={`w-6 h-1 mb-1 bg-[#1487B9] transition-all duration-200 ${isOpen ? 'rotate-45 translate-y-2' : ''}`} />
          <div className={`w-6 h-1 mb-1 bg-[#1487B9] transition-all duration-200 ${isOpen ? 'opacity-0' : ''}`} />
          <div className={`w-6 h-1 bg-[#1487B9] transition-all duration-200 ${isOpen ? '-rotate-45 -translate-y-2' : ''}`} />
        </div>
      )}
